"""Run one iteration of duplicate listmonk subscriber repair worker."""
...
async def _safe_write_heartbeat(extra: dict[str, object]) -> None:
    try:
        await write_heartbeat("listmonk-duplicate-subscriber", extra=extra)
    except Exception as exc:
        logger.warning(
            "listmonk_duplicate_subscriber_heartbeat_write_failed",
            error=str(exc),
...
async def _run() -> None:
    ...
    configure_logging(
        ...
        component=getattr(settings, "log_component", "listmonk-duplicate-subscriber"),
    )
        ...
        try:
            while not task.done():
                ...
                try:
                    ...
                    continue
            resolved = await task
            await _safe_write_heartbeat({"stage": "completed", "resolved": resolved})
            logger.info("listmonk_duplicate_subscriber_run_completed", resolved=resolved)
        except (ListmonkClientError, httpx.HTTPError) as exc:
            await _safe_write_heartbeat({"stage": "failed"})
            logger.error(
                "listmonk_duplicate_subscriber_run_failed",
                error=str(exc),
                error_type=type(exc).__name__,
            )
        finally:
