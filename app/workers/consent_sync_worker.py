...
def parse_list_ids(raw_list_ids: str) -> list[int]:
    """
    Parse a comma-separated string of ListMonk list IDs.
    
    Empty segments and values that cannot be converted to integers are ignored; whitespace around entries is trimmed.
    
    Parameters:
        raw_list_ids (str): Comma-separated list ID values (e.g. "1, 2,3").
    
    Returns:
        list[int]: Parsed list IDs as integers, in the original order.
    """
    result: list[int] = []
...
class ConsentSyncWorker:
    ...
    async def _process_pending_user(
        ...
        *,
        ...
        """
        Process a single pending mapping between an internal user and a Listmonk subscriber and advance consent, TEYCA fields, and bonus accrual state accordingly.
        
        This function checks the subscriber state (or uses subscriber_override), handles blocked and unconfirmed statuses, reserves and completes a consent bonus accrual, updates TEYCA pass fields, and persists progress and Listmonk check marks. It records metrics counters for notable outcomes and handles TeycaAPIError by saving failed progress and leaving the subscriber marked pending when appropriate.
        
        Parameters:
            pending (Any): Object containing at least `user_id` and `subscriber_id` identifying the internal user and Listmonk subscriber to process.
            target_list_ids (list[int]): List IDs that must be confirmed for consent to be considered complete.
            listmonk_repo (ListmonkUsersRepository): Repository used to mark subscribers as checked/confirmed in Listmonk.
            accrual_repo (BonusAccrualRepository): Repository used to reserve, update progress, and finalize bonus accrual operations.
            subscriber_override (SubscriberState | None): If provided, use this subscriber state instead of fetching it from Listmonk.
            metrics (ConsentSyncMetrics | None): Optional metrics accumulator to increment counters for events (e.g., not_found, blocked, confirmed).
        """
    ...
    async def run_once(self) -> int:
        """
        Process one batch of updated Listmonk subscribers for all configured target lists.
        
        Fetches deltas since the last watermark for each configured list, maps subscribers to internal users, processes each mapped subscriber (which may update TEYCA fields, reserve/accrue bonuses, and mark subscribers in Listmonk), updates per-list watermarks, persists changes, and aggregates per-run metrics.
        
        Returns:
            int: Number of mapped subscribers processed during this run.
        """
...
def build_consent_sync_worker() -> ConsentSyncWorker:
    """
    Builds a ConsentSyncWorker configured from the application's settings.
    
    Returns:
        ConsentSyncWorker: Worker initialized with the global session factory and clients for Listmonk and Teyca.
    """
...
def _initial_consent_payload(*, subscriber_id: int, list_ids: list[int]) -> dict[str, Any]:
    """
    Create the initial progress payload for a consent-sync accrual operation.
    
    Parameters:
    	subscriber_id (int): Internal Listmonk subscriber identifier.
    	list_ids (list[int]): Target Listmonk list IDs associated with this subscriber.
    
    Returns:
    	payload (dict[str, Any]): Dictionary containing `subscriber_id`, `list_ids`, and initial flags `bonus_done` and `key1_done` both set to `False`.
    """
    return {
        "subscriber_id": subscriber_id,
        "list_ids": list_ids,
...
def _normalize_progress_payload(
    *,
    ...
    """
    Ensure a progress payload contains required keys and normalizes their types.
    
    Parameters:
        raw_payload (dict[str, Any] | None): Optional existing progress payload to merge; may contain keys
            `subscriber_id`, `list_ids`, `bonus_done`, and `key1_done`.
        subscriber_id (int): Subscriber ID to use when `subscriber_id` is missing from raw_payload.
        list_ids (list[int]): List IDs to use when `list_ids` is missing from raw_payload.
    
    Returns:
        dict[str, Any]: A payload dict that always contains:
            - `subscriber_id` (int): normalized to int,
            - `list_ids` (list[int]): preserved or set to the provided list_ids,
            - `bonus_done` (bool): present and normalized to bool (defaults to False),
            - `key1_done` (bool): present and normalized to bool (defaults to False).
            Other keys from raw_payload are preserved.
    """
...
def _delta_to_state(delta: SubscriberDelta) -> SubscriberState:
    """
    Convert a Listmonk SubscriberDelta to a SubscriberState.
    
    Returns:
        SubscriberState: state with subscriber_id, status, list_ids, and list_statuses copied from the delta.
    """
...
def _inc(metrics: ConsentSyncMetrics | None, field_name: str) -> None:
    """
    Increment a named counter on the provided ConsentSyncMetrics instance.
    
    If `metrics` is None, the function is a no-op.
    
    Parameters:
    	metrics (ConsentSyncMetrics | None): Metrics object whose field will be incremented.
    	field_name (str): Name of the integer counter attribute on `metrics` to increment by 1.
    """
    if metrics is None:
        return
