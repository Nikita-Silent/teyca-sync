...
class ListmonkUser(Base):
    ...
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.user_id", ondelete="RESTRICT"), primary_key=True)
    ...
    consent_checked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class MergeLog(Base):
    ...
    merged_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, nullable=True)
...
class BonusAccrualLog(Base):
    ...
    __table_args__ = (
        UniqueConstraint("idempotency_key", name="uq_bonus_accrual_idempotency_key"),
    )
...
class SyncState(Base):
    ...
    __table_args__ = (
        UniqueConstraint("source", "list_id", name="uq_sync_state_source_list"),
    )
