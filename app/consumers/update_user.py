...
async def handle(payload: dict[str, Any], *, deps: UpdateConsumerDeps) -> None:
    """
    Process an UPDATE webhook payload and synchronize user state across internal repositories, Teyca, and Listmonk.
    
    Performs the following observable actions: validates and parses the webhook payload; optionally merges incoming profile data with legacy DB data and, if a merge occurs, records a merge log, updates Teyca pass fields, and accrues any legacy bonus; upserts the user profile in the users repository; blocks and marks records for invalid emails; upserts the subscriber in Listmonk and marks consent as pending.
    
    Parameters:
        payload (dict[str, Any]): Incoming webhook payload expected to validate against the WebhookPayload model.
        deps (UpdateConsumerDeps): Dependency container providing settings, repositories, and external service clients used to perform updates and side effects.
    """
...
def _to_optional_str(raw: object) -> str | None:
    """
    Coerce a value into a trimmed string when it contains non-whitespace characters.
    
    Parameters:
        raw (object): Value to coerce into a string.
    
    Returns:
        str | None: Trimmed string if `raw` is a `str` with non-whitespace characters, `None` otherwise.
    """
