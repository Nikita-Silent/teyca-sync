...
Цель: включить dispatcher worker'ы в runtime так, чтобы webhook-consumers перестали держать `RabbitMQ unacked` на внешних вызовах, и при этом не потерять side effect'ы в Listmonk/Teyca.
...
- Новые worker'ы `external-dispatcher-listmonk`, `external-dispatcher-merge`, `external-dispatcher-invalid-email` читают outbox по своим operation family, делают внешние вызовы и фиксируют локальный прогресс.
...
3. Одновременно добавить dispatcher worker'ы в runtime.
...
- `component="consumers"`
- `component="external-dispatcher-listmonk"`
- `component="external-dispatcher-merge"`
- `component="external-dispatcher-invalid-email"`
...
- dispatcher worker'ы включить сразу вместе с новым code rollout.
...
- Если `external_call_outbox` быстро растет, а `done` не растет:
  - проверить heartbeat `external-dispatcher-listmonk`, `external-dispatcher-merge`, `external-dispatcher-invalid-email`
...
1. Остановить dispatcher worker'ы.
