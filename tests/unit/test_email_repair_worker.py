...
def _worker() -> tuple[EmailRepairWorker, _WorkerMocks]:
    session = AsyncMock()
...
async def test_process_row_marks_manual_review_after_attempt_limit() -> None:
    ...
    mocks.listmonk_client.get_subscriber_by_email.side_effect = ListmonkClientError("boom")
...
async def test_process_row_marks_retry_when_teyca_cleanup_fails() -> None:
    ...
    mocks.listmonk_client.get_subscriber_by_email.return_value = SimpleNamespace(subscriber_id=777)
    listmonk_repo.get_by_subscriber_id.return_value = SimpleNamespace(user_id=20)
    mocks.teyca_client.update_pass_fields.side_effect = TeycaAPIError("teyca boom")
