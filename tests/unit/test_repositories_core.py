...
async def test_merge_log_repository_paths() -> None:
    """
    Test MergeLogRepository basic paths: existence check, creation, and deletion.
    
    Verifies that exists(user_id) reports presence when the query returns a row and absence when it does not; create(...) adds a MergeLog instance with the expected user_id to the session; delete_by_user_id(...) executes the repository's deletion path.
    """
    session = MagicMock()
    session.execute = AsyncMock()
    session.add = MagicMock()
