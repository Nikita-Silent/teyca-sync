...
def _worker() -> ListmonkReconcileWorker:
    """
    Constructs a ListmonkReconcileWorker configured with mocked session factory and client for tests.
    
    Returns:
        A ListmonkReconcileWorker preconfigured with:
        - settings: listmonk_list_ids="1" and consent_sync_batch_size=500
        - session_factory: a MagicMock that yields an AsyncMock session via an async context manager
        - listmonk_client: an AsyncMock client
    """
    session = AsyncMock()
...
async def test_reconcile_restores_deleted_subscriber_from_local_mapping() -> None:
    """
    Tests that a deleted Listmonk subscriber is restored using a local repository mapping.
    
    When no updated subscribers are returned by the Listmonk client and a local mapping exists for a deleted subscriber,
    the worker should restore the subscriber in Listmonk with the mapped email, list IDs, attributes, and desired status,
    upsert the restored subscriber mapping into the local repository with the new subscriber ID and status, and mark the
    associated user as having pending consent. Verifies the worker reports one restoration.
    """
    worker = _worker()
    worker.listmonk_client.get_updated_subscribers.return_value = []
