...
async def test_health_returns_ok_when_dependencies_are_available() -> None:
    """
    Verify the health endpoint reports overall OK and returns healthy per-check details when all dependencies are available.
    
    Asserts that a GET to "/webhook/health" responds with HTTP 200, an overall "ok" status, and checks containing:
...
async def test_live_and_ready_routes_are_split() -> None:
    """
    Verifies that the /webhook/live endpoint reports only the app heartbeat while /webhook/ready reports dependency health and fails when a dependency is unhealthy.
    
    Patches the database check as healthy, the RabbitMQ check as failing, and the app heartbeat as healthy; then asserts that:
    - /webhook/live returns 200 with only the app heartbeat check.
    - /webhook/ready returns 503 with database marked ok and rabbitmq marked error containing the failure message.
    """
    with patch(
        "app.api.webhook._check_database_health",
        new=AsyncMock(return_value=None),
